...
### 在视图之间移动
图层面板新增“视图”标签页，以树形列出图表中的所有视图，无需逐级返回即可直接跳转到任意视图。面板标题栏以面包屑显示当前位置的完整路径，每一级都可点击跳转。带有子视图的节点会在画布上显示标记，双击即可进入；视图切换带有缩放动画，能看出是进入内部还是退回外层。视图的重命名和删除也可从每行的菜单中完成。

### 深入查看详情视图
右键点击某个节点(移动端为长按)→"创建子视图",即可放大查看该项目自己的详情图。锚点图标始终固定且无法删除,因此你始终清楚当前查看的是谁的视图,图层面板的标题栏同时也充当返回按钮。图层面板中每个节点行现在也都有自己的创建/打开子视图图标。

### 更清晰的选中反馈
选中节点后,其他所有节点会变暗,让选中的节点更加突出;选中连接线后,画布上高亮显示的不再只是两端的端点,而是整条线本身;在图层面板中再次点击已选中的行会取消选中。

### 移动端响应式工具栏与设置
在窄屏幕上,顶部工具栏现在会自动切换为水平布局,与已保存的位置设置无关;设置面板在移动端也会全屏打开,并用可横向滚动的图标条导航取代侧边栏列表。
