...
### 뷰 사이 이동
레이어 패널의 "뷰" 탭이 다이어그램의 모든 뷰를 트리로 보여줘, 한 단계씩 올라가지 않고 원하는 뷰로 바로 이동할 수 있습니다. 패널 헤더에는 현재 위치의 전체 경로가 브레드크럼으로 표시되며 각 단계를 클릭해 이동할 수 있습니다. 하위 뷰가 있는 노드는 캔버스에서 표시가 나고 더블클릭으로 바로 들어갈 수 있으며, 뷰를 오갈 때 확대·축소 애니메이션이 붙어 안으로 들어갔는지 밖으로 나왔는지가 드러납니다. 뷰 이름 변경과 삭제도 각 행의 메뉴에서 할 수 있습니다.
