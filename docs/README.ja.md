...
### ビュー間の移動
レイヤーパネルの「ビュー」タブに図のすべてのビューがツリーで表示され、一段ずつ上がらなくても任意のビューへ直接移動できます。パネルのヘッダーには現在位置までの全パスがパンくずとして表示され、各段をクリックして移動できます。下位ビューを持つノードはキャンバス上に印が付き、ダブルクリックで直接入れます。ビュー間の移動にはズームのアニメーションが付き、内側に入ったのか外に出たのかが分かります。ビューの名前変更と削除も各行のメニューから行えます。

### 詳細ビューへのドリルダウン
ノードを右クリック(モバイルでは長押し)して「子ビューを作成」を選ぶと、そのアイテム専用の詳細ダイアグラムにドリルダウンできます。アンカーアイコンは固定されて削除できないため、今見ているビューが誰の詳細なのか常にわかり、レイヤーパネルのヘッダーが戻るボタンを兼ねます。レイヤーパネルの各ノード行にも、同じ機能の作成/表示アイコンが追加されました。

### 選択状態がより分かりやすく
ノードを選択すると他のすべてのノードが薄く表示され、選択中のノードが際立つようになりました。コネクターを選択すると、両端の点だけでなく線そのものもキャンバス上でハイライトされます。レイヤーパネルで既に選択されている行を再度クリックすると、選択が解除されます。

### モバイルでのツールバーと設定画面のレスポンシブ対応
狭い画面では、保存済みの位置設定に関わらず上部ツールバーが自動的に横並びレイアウトに切り替わるようになりました。設定画面もモバイルでは全画面表示になり、サイドバーの代わりに横スクロールするアイコンストリップのナビゲーションを使用します。

### 設定画面の刷新
検索(⌘K)対応の2ペイン構成の設定ダイアログが、従来のタブ方式に代わりました — ショートカット/パン/ズーム、表示、アイコンパック、拡張機能にグループ分けされ、キャンセル/保存方式とセクションごとの「デフォルトに戻す」ボタンを備えています。

### レイヤーパネル
検索と種類フィルター(すべて/ノード/コネクター/エリア/テキスト)、ノードが属するエリアの下へのネスト表示、項目ごとの表示/ロック切り替えに対応し、パネルは画面端にぴったりドッキングされ、ツールバーの位置も設定できるようになりました。

### ノード編集パネル
大きめのアイコンと選択バッジ、ゾーン/接続/タイプのチップを備えたサマリーカードに刷新され、ラベル表示モード(常に表示/ホバー時/非表示)と、クリックすると接続先のノードへキャンバスを移動する接続サマリーが追加されました。

### MCPツール連携
設定 → MCPパネルに、接続されたAIエージェントが呼び出せるツール(一覧取得/取得/作成/更新/パッチ/削除)の一覧が表示されるようになりました。

### アイコンパックのブランドロゴ
AWS、GCP、Azure、Kubernetesの各アイコンパックが、設定画面で汎用のイニシャルバッジではなく実際のブランドロゴを表示します。
